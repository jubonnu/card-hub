...
/**
 * 課金モーダル（Mobile-G4-3）。表示状態は`lib/paywallState.ts`の`derivePaywallState`が一元的に
 * 決定する。実際のpremium API許可はサーバー側で行われるため、購入・復元直後は必ず
 * `POST /me/entitlements/refresh`で照合してから「premium」状態へ遷移する
 * （5章「購入直後の即時照合」要件）。
 *
 * `app/paywall.tsx`（expo-routerの`presentation: 'modal'`画面）ではなくReact Native標準の
 * `Modal`として実装している。`app/_layout.tsx`で常時マウントし、`useGlobalModalStore`の
 * `paywallOpen`で開閉する。呼び出し側は`router.push('/paywall')`ではなく`openPaywall()`を使う。
 */
export function PaywallModal() {
  const theme = useTheme();
  const router = useRouter();
  const visible = useGlobalModalStore((s) => s.paywallOpen);
  const closePaywall = useGlobalModalStore((s) => s.closePaywall);
  ...
  useEffect(() => {
    if (visible) track('paywall_viewed');
  }, [visible]);
  ...
  }, [visible, billing.billingStatus, authStatus]);
  ...
  function handleGoToSignIn() {
    closePaywall();
    router.push('/sign-in' as Parameters<typeof router.push>[0]);
  }
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
            ...
            {state === 'signedOut' && (
              <View style={styles.section}>
                ...
                <PrimaryButton label="サインイン" size="md" onPress={handleGoToSignIn} />
              </View>
            )}
