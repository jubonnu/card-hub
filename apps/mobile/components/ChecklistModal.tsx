...
import { CheckCircleIcon, PencilIcon, PlusIcon } from '@/components/icons';
...
/**
 * チェックリストモーダル。`app/checklist/[lotteryId].tsx`（expo-routerの
 * `presentation: 'modal'`画面）ではなくReact Native標準の`Modal`として実装している。
 * `app/_layout.tsx`で常時マウントし、`useGlobalModalStore`の`checklistLotteryId`
 * （nullでなければ表示、値が対象のlotteryId）で開閉する。
 */
export function ChecklistModal() {
  const theme = useTheme();
  const lotteryId = useGlobalModalStore((s) => s.checklistLotteryId);
  const closeChecklist = useGlobalModalStore((s) => s.closeChecklist);
  const saved = useMyLotteriesStore((s) => (lotteryId ? s.getSaved(Number(lotteryId)) : undefined));
  const { groups, toggleStep, addStep, renameStep, ensureInitialized } = useChecklistStore();
  const steps = lotteryId ? (groups[lotteryId] ?? []) : [];
  ...
  return (
    <Modal
      visible={lotteryId !== null}
      animationType="slide"
      presentationStyle={Platform.OS === 'ios' ? 'pageSheet' : undefined}
      onRequestClose={closeChecklist}
    >
      <ScreenContainer edges={['top', 'bottom']}>
        <View style={styles.header}>
          <View style={styles.iconButton} />
          <Text style={[styles.headerTitle, { color: theme.colors.textPrimary }]}>チェックリスト</Text>
          <Pressable hitSlop={8} style={styles.iconButton} onPress={closeChecklist}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.stepsList}>
            {steps.map((step, index) => (
              <Pressable
                key={step.id}
                onPress={() => lotteryId && toggleStep(lotteryId, step.id)}
                style={[
                  styles.stepRow,
